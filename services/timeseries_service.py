...
class TimeSeriesService:
    ...
    def run(cls, job_id: int) -> dict[str, Any]:
        ...
        if steps < 1 or steps > 8760:
            raise ValueError("steps must be in [1, 8760]")
        ...
        try:
            ...
            # series[(table, ppi, var)] -> [v0, v1, ..., v_steps-1]
            series: dict[tuple[str, int, str], list[float]] = {}

            for t in range(steps):
                net = copy.deepcopy(base_net)

                # Apply load scaling for this step
            ...
            db.session.commit()
            return {"job_id": job.id, "steps": steps, "tracked": len(series)}
    ...
    # -----------------------------------------------------------------
    @staticmethod
    def _apply_load_profiles(net, profiles: list, step: int) -> None:
        """
        profiles is a list of {"pp_index": int, "values": [list of scaling factors]}.
        If a load is not addressed by any profile, it stays at base scaling.
        """
        for p in profiles:
            ppi = p.get("pp_index")
    ...
    def _collect_step(net, variables, series, step) -> None:
        for var in variables:
            if var in ("vm_pu", "va_degree"):
                ...
                # Collect from both lines and trafos under their respective keys
